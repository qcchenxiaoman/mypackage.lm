...
`summaryLM()` is a function that prints the summary of the output from `LM()`. The default dataset  `iris` is used for examples and tutorials. 

### Usage of function `LM()` and `summaryLM()`:

In this section, we introduce the usage of `LM` and `summaryLM()`.  

#### Call the function `LM()` and obtain the outputs relate to the fitted model
...
Both the build-in methods `lm()` and `summary(lm())` returns the related results from a fitted linear regression model, but each method would return different kinds of results. Therefore, the function `LM()` combine those results, which includes the essential results from both methods. 
